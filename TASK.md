...
- **comment** (опционально): человекочитаемое сообщение, например, суть вопроса клиента

#### 2. `insufficient_data`
- **Семантика**: Намерение определено, но не хватает обязательных параметров
- **intent**: Определенное намерение (например, `"send_email"`)
- **data**: Объект с информацией о недостающих полях
- **comment** (опционально): человекочитаемое сообщение о том, что требуется
...
- **comment** (опционально): человекочитаемое сообщение, объясняющее почему намерение не определено
...
```json
{
  ...
  "data": {
    ...
    "comment": "Для отправки письма необходимо указать получателя"
  }
}
```
...
- `data.comment` (опциональное): человекочитаемое сообщение о том, что требуется
...
| **Структура data** | Пустой объект или `comment` | Информация о недостающих полях (`missing_required_fields`, `comment`) |
| **Поле comment** | Опционально, для объяснения | Опционально, для просьбы уточнить данные |
