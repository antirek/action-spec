...
- После извлечения данных, ОБЯЗАТЕЛЬНО проверь наличие ВСЕХ полей с required: true
...
4. Если все обязательные поля присутствуют → status: "success"
5. Если отсутствует хотя бы одно обязательное поле → status: "insufficient_data"
6. Верни результат в формате JSON
...
- **"success"**: Намерение успешно определено И ВСЕ обязательные поля (required: true) присутствуют в data
  - `intent`: определенное намерение
  - `data`: объект с данными намерения (ключи соответствуют полям field из структуры намерения)
  - **КРИТИЧЕСКИ ВАЖНО:** Используй этот статус ТОЛЬКО если все поля с `required: true` присутствуют в data
  - НЕ возвращай status: "success" если отсутствует хотя бы одно обязательное поле

- **"insufficient_data"**: Намерение определено, но отсутствует хотя бы одно обязательное поле (required: true)
  - `intent`: определенное намерение
  - `data`: объект с полями:
    - `missing_required_fields` (массив строк, обязательное): список ВСЕХ недостающих обязательных полей
    - `comment` (строка, опционально): человекочитаемое сообщение о том, что требуется
  - **КРИТИЧЕСКИ ВАЖНО:** Используй этот статус если намерение определено, но отсутствует хотя бы одно обязательное поле
...
3. **Валидация обязательных полей (КРИТИЧЕСКИ ВАЖНО):**
   - После извлечения данных, ОБЯЗАТЕЛЬНО проверь наличие ВСЕХ полей с `required: true`
   - Пройдись по каждому полю в структуре data намерения
   - Для каждого поля с `required: true` проверь, присутствует ли оно в извлеченных данных
   - Если ВСЕ обязательные поля присутствуют → верни status: "success"
   - Если отсутствует ХОТЯ БЫ ОДНО обязательное поле → верни status: "insufficient_data"
   - В data.missing_required_fields укажи массив ВСЕХ недостающих обязательных полей
   - НЕ возвращай status: "success" если отсутствуют обязательные поля
...
1. **Проверка обязательных полей:**
   - После определения намерения и извлечения данных, ОБЯЗАТЕЛЬНО проверь все поля с `required: true`
...
3. **Статус insufficient_data:**
   ...
   - В data.missing_required_fields укажи массив всех недостающих обязательных полей
   - В data.comment (опционально) укажи человекочитаемое сообщение о том, что требуется
...
- Если значение обязательного поля не может быть определено - НЕ включай его в результат и верни status: "insufficient_data"
- Если значение необязательного поля не может быть определено - не включай его в результат
...
### Пример 2: Недостаточно данных (отсутствуют все обязательные поля)
...
### Пример 2а: Недостаточно данных (отсутствует часть обязательных полей)
Запрос: "Отправь письмо на адрес user@example.com"
Результат:
{
  "status": "insufficient_data",
  "intent": "send_email",
  "data": {
    "recipient": "user@example.com",
    "missing_required_fields": ["subject"],
    "comment": "Для отправки письма необходимо указать тему"
  }
}
**Примечание:** Даже если часть обязательных полей присутствует, но отсутствует хотя бы одно - используй status: "insufficient_data"
...
4. **Валидация ответа:**
   ...
   - **КРИТИЧЕСКИ ВАЖНО:** Если status: "success", убедись что ВСЕ обязательные поля присутствуют
   - Если status: "success" но отсутствуют обязательные поля - это ошибка промпта, нужно усилить инструкции
...
6. **Усиление промпта для надежной валидации:**
   - Используй расширенную версию промпта с разделом "КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА ВАЛИДАЦИИ"
   - Добавляй явные инструкции о проверке обязательных полей
   - Включай примеры с недостающими полями для лучшего понимания AI
   - Если AI возвращает success без обязательных полей - усиль инструкции по валидации
