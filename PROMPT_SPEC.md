...
2. **Пользовательский запрос** (user message) - запрос пользователя и список намерений
...
- **"insufficient_data"**: Намерение определено, но отсутствуют обязательные параметры
  ...
  - `data`: объект с полями:
    ...
    - `message` (строка, опционально): человекочитаемое сообщение о том, что требуется
...
Результат:
{
  ...
  "data": {
    ...
    "message": "Для отправки письма необходимо указать получателя и тему"
  }
}
...
Пользовательский запрос должен содержать:
...
- `message` - опциональное поле с человекочитаемым сообщением
...
```json
{
  ...
  "data": {
    ...
    "message": "Для создания задачи необходимо указать название"
  }
}
```
...
1. **Формирование списка намерений:**
